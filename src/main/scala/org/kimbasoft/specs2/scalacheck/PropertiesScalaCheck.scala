...
object PropertiesScalaCheck extends App {
  ...
  println("-- Simple Properties ---------------------------------")
  /* Defining a Prop with just an Int as input (values are generated by default Int Gen) */
  println("\n\"ForAll\" Square Root Prop: ")
  val propSqrt1 = forAll { (n: Int) => sqrt(n * n) == n }
  propSqrt1.check

  /* Defining a Prop with two containers as input (values are generated by default Int Gen for lists) */
  println("\n\"ForAll\" Concatenate List Prop: ")
  val propConcat = forAll { (l1: List[Int], l2: List[Int]) => l1.size + l2.size == (l1 ::: l2).size }
  propConcat.check

  /* Providing a custom Test Data Generator to use with the Prop */
  println("\n\"ForAll\" Square Root Prop (custom Gen): ")
  val genSqrt2 = Gen.choose(1, 100)
  val propSqrt2 = forAll(genSqrt2) { (n: Int) => scala.math.sqrt(n * n) == n }
  propSqrt2.check

  /* Restricting Test Data by using an implication: (implication) ==> (Prop) */
